...
Sources
-------
``LARGE`` values were originally taken from the ACR MRI Quality Control
Manual (2015 edition) and have not been updated in this revision in order
to preserve the historical pass/fail behaviour against datasets that have
already been validated against this engine. See the README notes for known
differences between these values and the 2022 ACR Large and Medium Phantom
Test Guidance.

``MEDIUM`` values come from the **ACR Large and Medium Phantom Test
Guidance, 10.19.2022** (file ``MR_ACR_Large_Med_Phantom_Guidance_102022``
at the project root). Specific section references are noted on each
field. Where the 2022 guidance lists separate ACR-T1 and ACR-T2 LCD
thresholds for the same field-strength band, the engine uses the stricter
of the two (T1) since the current run() loop applies a single threshold
per series.
"""
...
LARGE = PhantomSpec(
    ...
    piu_large_roi_area_cm2=200.0,
    piu_small_roi_area_cm2=1.0,
    piu_threshold_lowfield_percent=82.0,
    piu_threshold_3t_percent=87.5,
